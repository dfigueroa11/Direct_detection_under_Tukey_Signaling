...
k_tx = [4]*N_sym_blocks#rng.choice(len(class_rep_block.representative_class), N_sym_blocks)
...
k_rx = detector_block.decode_liklyhood(y,z,N_sym_blocks)
e = time.time()
...
noise_power = 10*np.log10(sig_power)+30
print(f"signal power: {sig_power: .2f} dBm")
print(f"noise power: {noise_power: .3e} dBm")
print(f"simulation time: {e-s: .3f}")
print(f"SER: {np.mean(k_tx != k_rx): .3e}")
